#!/usr/bin/env python3
"""
2-DOF Ankle Joint Torque Solver (Python)
=========================================
...
# ═══════════════════════════════════════════════════
# Topology (from CAD):
#   Motor ① (top):  shaft along Y, crank extends RIGHT (+X)
#   Motor ② (bottom): shaft along Y, crank extends LEFT (−X)
#   θ=0: crank UP (+Z)
#   Ankle attachment RIGHT: (+E/2, 0, +h)
#   Ankle attachment LEFT:  (−E/2, 0, +h)
...
class Mech:
    ...
    def set(self, p):
        ...
        self.a1b = Vec3(p['E']/2, self.oy, p['ah'])    # RIGHT
        self.a2b = Vec3(-p['E']/2, self.oy, p['ah'])   # LEFT
        self.ac = Vec3(0, 0, p['ujZ'])            # U-joint center
...
if __name__ == '__main__':
    params = {'m1z':23.5, 'm2z':16.0, 'r1':2.5, 'r2':2.5, 'L1':20.7, 'L2':12.7, 'E':2.3, 'ujZ':3.6, 'ah': 0.0, 'oy':3.8}
    mech = Mech(params)

    # Verify neutral IK angles match original simulation
    t1, t2, ok = mech.ik(0, 0)
    print("=" * 65)
    print("  Ankle Torque Solver — Verification")
    print("=" * 65)
    print(f"  Neutral IK angles: θ₁ = {t1*RAD:.1f}°, θ₂ = {t2*RAD:.1f}° {'✓' if ok else '✗'}")
    print()
    ...
    for pitch_deg, roll_deg, tp, tr, desc in test_cases:
        p_rad = pitch_deg * DEG
        r_rad = roll_deg * DEG
        res = mech.solve_torques(p_rad, r_rad, tp, tr)
        print(f"  [{desc}]")
        print(f"    Pose: Pitch={pitch_deg}°, Roll={roll_deg}°")
        print(f"    Load: τ_pitch={tp} N·m, τ_roll={tr} N·m")
        if res:
            print(f"    Motor ① θ₁={res['t1']*RAD:.1f}°  →  τ₁ = {res['m1']:+.3f} N·m")
            print(f"    Motor ② θ₂={res['t2']*RAD:.1f}°  →  τ₂ = {res['m2']:+.3f} N·m")
            print(f"    Jacobian det = {res['det']:.6f}, κ = {res['cn']:.1f}")

            # Motor compatibility
            eff_max = max(abs(res['m1']), abs(res['m2']))
            for name, spec in MOTORS.items():
                if eff_max <= spec['rated']:
                    status = '🟢 Safe (within rated)'
                elif eff_max <= spec['peak']:
                    status = '🟡 Caution (within peak)'
                else:
                    status = '🔴 OVERLOAD'
                print(f"      {name}: {status}")
        else:
            print(f"    ✗ FAILED (unreachable or singular)")
        print()
